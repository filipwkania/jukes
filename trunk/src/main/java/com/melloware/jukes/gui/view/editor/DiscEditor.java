...
/**
 * An implementation of {@link Editor} that displays a {@link Disc}.<p>
 *
...
public final class DiscEditor
    ...
    public void build() {
        ...
        FormLayout layout = new FormLayout("fill:pref:grow", "p, 12px, p, p, 12px, p, 12px, p, 12px, p");
        ...
        if (this.getSettings().isAuditInfo()) {
        	builder.addSeparator(Resources.getString("label.auditinfo"), cc.xy(1, 8));
     ...
     */
    public void commit() {
      /** AZ **/  commit(this.getSettings().isUpdateTags());
    }
    ...
    public void findCover() {
        Disc disc = getDisc();
        if (disc.isNotValid()) {
           	final String errorMessage = ResourceUtils.getString("messages.discnotexists");
		...
		try {
			imageLocation = ImageFactory.saveImageToUserDefinedDirectory(file, 
			    ...
			    disc.getName().toString(),
			    disc.getYear().toString());
		} catch (IOException e) {
        ...
        try {
            ...
            updateView();
           	...
           	final String errorMessage = ResourceUtils.getString("messages.DiscUnique"); 
            MessageUtil.showError(this, errorMessage); //AZ
            LOG.error(errorMessage, ex);
            HibernateUtil.rollbackTransaction();
            HibernateDao.refresh(disc);
        } catch (Exception ex) {
           	final String errorMessage = ResourceUtils.getString("messages.ErrorUpdatingCoverImage"); 
            MessageUtil.showError(this, errorMessage); //AZ
            LOG.error(errorMessage, ex);
            HibernateUtil.rollbackTransaction();
            HibernateDao.refresh(disc);
        ...
        try {
            ...
            for (Iterator iter = getDisc().getTracks().iterator(); iter.hasNext();) {
                ...
                if (file.exists()) {
                    ...
                    if (musicTag.renameFile(this.getSettings().getFileFormatMusic())) {
    ...
    public void rollback() {
        ...
        try {
            ...
            super.rollback();
           	...
           	final String errorMessage = ResourceUtils.getString("messages.ErrorRefreshingDisc")+ ": "+ getDisc().getName(); 
            MessageUtil.showError(this, errorMessage);
            LOG.error(errorMessage, ex);
        } finally {
            setBusyCursor(false);
    ...
    /**AZ
     * Performs the FreeDB service search.
     */
    public void freeDBSearch() {
        ...
        Iterator it;
        final FreeDBDialog dialog = new FreeDBDialog(getMainFrame(), getSettings());
    ...
    protected void updateModel() {
        ...
        if (!StringUtils.equalsIgnoreCase(disc.getGenre(), (String)genre.getSelectedItem())) {
            disc.setGenre((String)genre.getSelectedItem());
        }

        if (!StringUtils.equalsIgnoreCase(disc.getNotes(), notesField.getText())) {
            disc.setNotes(notesField.getText());
        }     
        /** AZ set Artist **/
        //AZ - Check for empty Artist field. In addition to standard Validation method.
        if ((artistField.getText()== null) ||  (ValidationUtils.isBlank(artistField.getText()))) {
           	final String errorMessage = ResourceUtils.getString("messages.UpdateModel")+ Resources.MESSAGE_EDITOR_ERRORS
           								+ "\n " + ResourceUtils.getString("messages.ArtistNameMandatory");
            MessageUtil.showError(this, errorMessage); //AZ
            LOG.error(errorMessage);
        } else if (!StringUtils.equalsIgnoreCase(disc.getArtist().getName() , artistField.getText())) {
         	// try to persist
            try {
                setBusyCursor(true);
                ...
                final String hql = MessageFormat.format(resource,
                                                        new Object[] { StringEscapeUtils.escapeSql(artistField.getText()) });
                foundArtist = (Artist)HibernateDao.findUniqueByQuery(hql);
                // If Artists does not exist in database
                if (foundArtist == null) {
                   	final String errorMessage = ResourceUtils.getString("messages.ErrorUpdatingArtistNotFound") + artistField.getText();
                	LOG.error(errorMessage);
                    MessageUtil.showError(this, errorMessage); //AZ
                }
                if (foundArtist != null) {
    ...
    public void webSearch() {
        ...
        final WebSearchDialog dialog = new WebSearchDialog(getMainFrame(), getSettings());
        dialog.setSelectedArtist(disc.getArtist().getName());
        dialog.setSelectedDisc(disc.getName());
        dialog.open();
        ...
        if (dialog.getSelectedImage() != null) {
            try {
                if (StringUtils.isNotBlank(disc.getCoverUrl())) {
                    ImageFactory.saveImage(dialog.getSelectedImage(), disc.getCoverUrl());
                } else {
                	String filename = disc.getLocation() + SystemUtils.FILE_SEPARATOR + "cover." + ImageFilter.JPG;
                	ImageFactory.saveImage(dialog.getSelectedImage(), filename);
                    disc.setCoverUrl(filename);
                }
                albumImage.setImage(ImageFactory.getDiscImage(disc.getCoverUrl()).getImage());
                //Save copy image
                final File coverFile = new File(disc.getCoverUrl());
                ImageFactory.saveImageToUserDefinedDirectory(coverFile,
                											disc.getArtist().getName(), 
                											disc.getName(),
                											disc.getYear());
    ...
    protected void updateView() {
        ...
        for (Iterator iter = disc.getTracks().iterator(); iter.hasNext();) {
            ...
            final String name = track.getDisplayText(getSettings().getDisplayFormatTrack());
            final JukesValidationMessage message = new JukesValidationMessage(name, null, track);
            tracks.add(message);
        }
        Collections.sort(tracks, TrackListCellRenderer.TRACK_COMPARATOR);
        for (Iterator iter = tracks.iterator(); iter.hasNext();) {
            JukesValidationMessage message = (JukesValidationMessage)iter.next();
    ...
    private JComponent buildDiscPanel() {
        ...
        FormLayout layout = new FormLayout("right:max(14dlu;pref), 4dlu, left:min(80dlu;pref):grow, pref, 4dlu, pref, 25px",
        ...
        builder.addLabel(Resources.getString("label.name") + ": ", cc.xy(1, 3));
        builder.add(nameField, cc.xyw(3, 3, 3));
        builder.add(ComponentFactory.createTitleCaseButton(nameField), cc.xy(6, 3));
        ...
        builder.addLabel(Resources.getString("label.notes") + ": ", cc.xy(1, 11, "left,top"));
    ...
    private void commit(boolean aUpdateTags) {
        super.commit();
        final Disc disc = getDisc();
        final String temporaryDiscName = disc.getName(); //AZ Store disc to rollback
        ...
        final String oldImageName = ImageFactory.standardImageFileName(disc.getArtist().getName(), disc.getName(), disc.getYear());
        Artist foundArtist = null;
        Artist artist = null;
        //AZ - Check for empty Artist field. In addition to standard Validation method.
        if ((artistField.getText()== null) ||  (ValidationUtils.isBlank(artistField.getText()))) {
        ...
        boolean artistChanged = (!StringUtils.equalsIgnoreCase(disc.getArtist().getName() , artistField.getText()));
        ...
        try {
            ...
            // if artist was found 
            	LOG.debug("DiscEditor - Change Artist, Artist was found in Database: " + artistField.getText());
            }            
        } catch (InfrastructureException ex) {
           	final String errorMessage = ResourceUtils.getString("messages.ArtistNameUnique"); 
            MessageUtil.showError(this, errorMessage); //AZ
        ...
        try {
            ...
            if (this.getSettings().isCopyImagesToDirectory()) {          
             File oldImageFile = new File (oldImageName); 
              if ( oldImageFile.exists() ) {
                  final String newImageName = ImageFactory.standardImageFileName(artistField.getText(), disc.getName(), disc.getYear());
            ...
            rollback();
            hasErrors = true;
            getValidationModel().setDirty(false);//Set Dirty false to lock editor withour changes
        } catch (Exception ex) {
           	final String errorMessage = ResourceUtils.getString("messages.ErrorUpdatingDisc"); 
            MessageUtil.showError(this, errorMessage); //AZ
            LOG.error(errorMessage, ex);
            HibernateUtil.rollbackTransaction();
            HibernateDao.refresh(disc);
        ...
        if (!hasErrors) {
            if (aUpdateTags) {
                ...
                progressMonitor = new ProgressMonitor(getMainFrame(), Resources.getString("messages.updatetracks"), "",
                                                      0, (int)task.getLengthOfTask());
    ...
    private void initComponentAnnotations() {
        ...
        ValidationComponentUtils.setInputHint(year, Resources.getString("messages.YearIsMandatory"));
        ValidationComponentUtils.setMandatory(year, true);
        ValidationComponentUtils.setMessageKey(year, "Disc.Year");
        ValidationComponentUtils.setInputHint(genre, Resources.getString("messages.GenreIsMandatory"));
        ...
        /** AZ **/
        ValidationComponentUtils.setInputHint(artistField, Resources.getString("messages.ArtistIsMandatory"));
        ValidationComponentUtils.setMessageKey(artistField, "Artist.Name");
        ValidationComponentUtils.setMandatory(artistField, true); 
        
        ValidationComponentUtils.setInputHint(genreEditor, Resources.getString("messages.GenreIsMandatory"));
        }
    ...
    private void initComponents() {
        ...
        ArtistValidationModel artistValidationModel = new ArtistValidationModel (new Artist());

        headerToolbar = buildToolBar();
        nameField = ComponentFactory.createTextField(validationModel.getModel(Disc.PROPERTYNAME_NAME), false);
        /** AZ **/
        artistField = ComponentFactory.createTextField(artistValidationModel.getModel(Artist.PROPERTYNAME_NAME), false);
        
        year = ComponentFactory.createTextField(validationModel.getModel(Disc.PROPERTYNAME_YEAR), false);
        ((JTextField)year).setColumns(5);
        final ValueModel genreChoiceModel = validationModel.getModel(Disc.PROPERTYNAME_GENRE);
        genreLabel = new JLabel();
        genreLabel.setName("GENRE");
        //genre = ComponentFactory.createComboBox(new SelectionInList(GENRE_LIST, genreChoiceModel));
        //AZ - Constructs an editable JComboBox for the specified List of Genres
        ComboBoxAdapter genreAdapter = new ComboBoxAdapter(GENRE_LIST.toArray(), genreChoiceModel); //AZ
        genre = new JComboBox(genreAdapter); //AZ
        genre.setEditable(true); //AZ  
        //AZ set sub-component of ComboBox - ComboBoxEditorTextField - as JComponent
        genreEditor = (JComponent)genre.getComponent(2);//AZ
       
        notesField = ComponentFactory.createTextArea(getValidationModel().getModel(Disc.PROPERTYNAME_NOTES), false);
        notesField.setLineWrap(true);
        notesField.setWrapStyleWord(true);
        notesField.setRows(3);
        location = ComponentFactory.createLabel(getValidationModel().getModel(Disc.PROPERTYNAME_LOCATION));
        albumImage = new AlbumImage(new Dimension(this.getSettings().getCoverSizeLarge(), this.getSettings().getCoverSizeLarge()));
        ...
        trackList.setCellRenderer(new TrackListCellRenderer(this.getSettings()));
        final JPopupMenu popup = MainMenuBuilder.buildPlayerPopupMenu(trackList);
        MouseListener popupListener = new ListPopupListener(popup);
        trackList.addMouseListener(popupListener);
        trackList.add(popup);
        listScrollPane = new SimpleInternalFrame(Resources.getString("label.tracks"));
        listScrollPane.setFrameIcon(Resources.TRACK_TREE_ICON);
        listScrollPane.setContent(UIFactory.createStrippedScrollPane(trackList));
