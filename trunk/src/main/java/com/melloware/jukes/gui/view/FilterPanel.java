...
public final class FilterPanel extends ComplexInternalFrame implements ActionListener {
   ...
   private final JCheckBox exactFlag; // AZ
   ...
   public FilterPanel(Settings settings) {
      ...
      genreField.setEditable(true); // AZ
   ...
   public void applyFilter() {
      ...
      try {
         ...
         if (StringUtils.isNotBlank(artist)) {
            final String artistStmt = artist.replaceAll("'", "\'\'");// replace
                                                                     // Single
                                                                     // Quotes
                                                                     // in
                                                                     // artist.name
         ...
         if (StringUtils.isNotBlank(album)) {
            final String albumStmt = album.replaceAll("'", "\'\'");// replace
                                                                   // Single
                                                                   // Quotes in
                                                                   // album.name
            if (exactFlag.isSelected()) {
         ...
         if ((StringUtils.isNotBlank(genre)) && (!StringUtils.equals(genre, GENRE_NONE))) {
            // OLD : query = query + " and disc.genre = '" + genre + "'";
            if (exactFlag.isSelected()) {
         ...
         if (bitrateField.getSelectedItem() != null) {
            query = query + " and disc.bitrate  " + operatorBitrate.getSelectedItem().toString()
         ...
         if (newFlag.isSelected()) {
            ...
            query = query + " and disc.createdDate > '" + format.format(new Date(timespan.getTime())) + " 00:00:00'";// AZ
         }
   ...
   private JComponent buildFilter() {
      FormLayout layout = new FormLayout("3px, pref, pref, pref, pref, fill:pref:grow",
               "4px, p, 4px, p, 4px, p, 4px, p, 4px, p, 4px, p, 4px, p");
      PanelBuilder builder = new PanelBuilder(layout);
      CellConstraints cc = new CellConstraints();

      builder.addLabel(Resources.getString("label.artist") + ": ", cc.xy(2, 2));// AZ
      builder.add(artistField, cc.xyw(3, 2, 4));// AZ
      builder.addLabel(Resources.getString("label.disc") + ": ", cc.xy(2, 4));// AZ
      builder.add(albumField, cc.xyw(3, 4, 4));// AZ
