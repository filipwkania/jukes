...
public final class Mp3Tag
    ...
    private String CODE_ALBUM_ARTIST = "ALBUM ARTIST"; //AZ
    private String V2_CODE_DISC_NUMBER = "TPOS";  //AZ
    ...
    public Mp3Tag(File aFile)
        ...
        try {
            ...
            throw new MusicTagException("IOException opening Music File Tag. " + aFile.getAbsolutePath() + LINE_BREAK
                                       + ex.getMessage());
        } catch (TagException ex) {
            LOG.error("TagException "+ex.getMessage(), ex);
            throw new MusicTagException("MusicTagException opening Music File Tag. " + aFile.getAbsolutePath() + LINE_BREAK
                                        + ex.getMessage());
        } /** AZ **/
          catch (InvalidAudioFrameException ex) {
            LOG.error("InvalidAudioFrameException "+ex.getMessage(), ex);
            throw new MusicTagException("InvalidAudioFrameException opening Music File Tag. " + aFile.getAbsolutePath() + LINE_BREAK
                                        + ex.getMessage());
        }
          catch (RuntimeException ex) {
            LOG.error("RuntimeException "+ex.getMessage(), ex);
            throw new MusicTagException("RuntimeException opening Music File Tag. " + aFile.getAbsolutePath() + LINE_BREAK
                                        + ex.getMessage());
        } catch (Exception ex) {
    ...
    public String getArtist() {
	   	/** AZ - get settings for MainModule for Application **/
    ...
    public Long getBitRate() {
        if (this.bitRate == null) {
            final Integer bitrate = (header == null) ? Integer.valueOf(1000)
                                                     : ((Integer)header.get("mp3.bitrate.nominal.bps"));
    ...
    public String getDisc() {
        ...
        if (StringUtils.isBlank(this.disc)) {
            ...
            /** AZ 
            Add Number of disc (TPOS) to the disc name **/
    ...
    public String getGenre() {
        if (StringUtils.isBlank(this.genre)) {
            ...
            //AZ process Genres string of iTunes type "(NNN)"
              ...
              if (genreString.indexOf(")") != -1) {
            	try {
                    ...
                    this.genre = getStandardGenreType(i);
                	  ...
                	  MessageUtil.showError(mainFrame, "NumberFormatException: " + nfe.getMessage()); //AZ 
                 }         	
              }
            }//end of processing Genres string of iTunes type "(NNN)"
            
            //AZ remove last char if char(00) which is added by foobar player
    ...
    public String getTitle() {
    	...
    	// if not blank then just return the title
        ...
        if (audioFile.hasID3v1Tag()) {
            ...
            // if this is the max length for a v1 tag, or it begins with "Track"
    ...
    public String getTrack() {
        if (StringUtils.isBlank(this.track)) {
            ...
            if ((audioFile.hasID3v1Tag()) && (audioFile.getID3v1Tag() instanceof ID3v11Tag)) {
                ...
                // default it to  non number if empty so isNumeric() test fails
            ...
            // if V1 is a number grab it as a number
            int track1 = 0;
            int track2 = 0;
            ...
            // v2 can contain format like 7 of 13, 12/15, etc.
            if (StringUtils.isNumeric(v2)) {
                track2 = Integer.valueOf(v2).intValue();
            }
            // find the best match which is the larger of these two values
    ...
    public boolean renameFile(final String aFormat) {
        ...
        try {
        	...
        	final MainFrame mainFrame = (MainFrame) Application.getDefaultParentFrame();
        	MessageUtil.showError(mainFrame, "TagException"); //AZ 
            LOG.error("TagException", ex);
        } catch (ReadOnlyFileException ex) {
        	final MainFrame mainFrame = (MainFrame) Application.getDefaultParentFrame();
        	MessageUtil.showError(mainFrame, "ReadOnlyFileException"); //AZ 
            LOG.error("ReadOnlyFileException", ex);
      } catch (InvalidAudioFrameException ex) {
    	  final MainFrame mainFrame = (MainFrame) Application.getDefaultParentFrame();
    	  MessageUtil.showError(mainFrame, "InvalidAudioFrameException"); //AZ 
    ...
    private void initializeTags() {
        // check for V1 tag, if not there then create one, if V0 then convert it to V11
        if (audioFile.hasID3v1Tag()) {
            if (audioFile.getID3v1Tag().getMajorVersion() == 0) {
                ...
                // Set to the correct tag need to cast
        ...
        // check for V2 tag and set it as the default V2 tag in this file
        if (audioFile.hasID3v2Tag()) {
        ...
        this.getTrackLength();
        this.getEncodedBy();
        // now create the new V2 tag to fill and delete the old V2 tag
    ...
    */
    private AbstractID3v2Frame loadFrame(final String aFrameCode) {
        ...
        // look for the correct frame in the tag
        if (audioFile.hasID3v2Tag()) {
            ...
            if (frames instanceof AbstractID3v2Frame) {
                ...
                // more than one frame of this type so just return the first one
        ...
        // if no frame was found create a new one then
        if (frame == null) {
