...
public final class ArtistEditor
    ...
    public void build() {
        ...
        FormLayout layout = new FormLayout("fill:pref:grow",
                                           "max(14dlu;pref), 3dlu, p, 4px, p, 4px, p, 4px, p, 4px, p, 4px, p");
    ...
    public void delete() {
        ...
        try {
            ...
            //      then delete the image copy for all discs
    ...
    protected void updateView() {
        ...
        // create random number generator to randomly pick 5 covers
        Random random = new Random();

        Collection discs = this.getDiscCollection();
        // grab till we have 5 random numbers
    ...
    private JComponent buildArtistPanel() {
        FormLayout layout = new FormLayout("right:max(14dlu;pref), 4dlu, left:min(80dlu;pref):grow, pref, 4dlu, 25px",
                                           "p, 4px, p, 7px");
    ...
    private void commit(boolean aUpdateTags) {
        ...
        try {
            ...
            HibernateUtil.beginTransaction();
            if (foundArtist != null) {
                // if artist was found add all discs under the new artist
                for (Iterator iter = artist.getDiscs().iterator(); iter.hasNext();) {
                    Disc disc = (Disc)iter.next();
            ...
            HibernateUtil.commitTransaction();
            // AZ : If transaction is committed and copies of images are used
            //      then change the name of the image copy for all discs
            if (this.getSettings().isCopyImagesToDirectory()) {
              for (Iterator iter = artist.getDiscs().iterator(); iter.hasNext();) {
                Disc disc = (Disc)iter.next();
    ...
    private void initComponents() {
        ...
        final Dimension dimension = new Dimension(this.getSettings().getCoverSizeSmall(),
                                            this.getSettings().getCoverSizeSmall());
