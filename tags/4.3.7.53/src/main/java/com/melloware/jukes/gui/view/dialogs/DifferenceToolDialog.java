...
/**
 * Compares a text file list of a catalog against the current catalog loaded in
 * the database. Useful for comparing what one catalog has that the other one
 * doesn't or what they have in common.
 * <p>
...
public final class DifferenceToolDialog extends AbstractDialog {
   ...
   protected JComponent buildHeader() {
      return new HeaderPanel(Resources.getString("label.DifferenceAnalyzerTool"), Resources
               .getString("label.DifferenceAnalyzerToolMessage"), Resources.DIFFERENCE_TOOL_ICON);
   }
   ...
   protected JComponent buildMainPanel() {
      ...
      Action intersection = new AbstractAction(Resources.getString("label.Intersection"),
               Resources.DIFFERENCE_TOOL_INTERSECTION_ICON) {
         // This method is called when the button is pressed
         public void actionPerformed(ActionEvent evt) {
            doCompare(evt);
         }
   ...
   protected void save(ActionEvent aEvt) {
      ...
      try {
         ...
         FileUtils.writeLines(file, "UTF-8", results); // AZ: implementation of
                                                       // UTF-8
   ...
   private JComponent buildFilePanel() {
      ...
      final FormLayout layout = new FormLayout(
               "right:max(14dlu;pref), 4dlu, left:min(60dlu;pref):grow, pref, 40dlu, pref, pref", "p");
      final PanelBuilder builder = new PanelBuilder(layout);
      final CellConstraints cc = new CellConstraints();

      builder.addLabel(Resources.getString("label.SelectFile"), cc.xy(1, 1));
      builder.add(file, cc.xyw(3, 1, 3));
      builder.add(ComponentFactory.createFileChooserButton(file, Resources.getString("label.SelectFile"), FilterFactory
               .textFileFilter()), cc.xy(6, 1));
   ...
   private JComponent buildOptionsPanel() {
      ...
      final FormLayout layout = new FormLayout("right:max(14dlu;pref), 4dlu, fill:pref:grow", "p, 4px, p"); // extra
                                                                                                            // bottom
                                                                                                            // space
                                                                                                            // for
                                                                                                            // icons
    ...
    */
   private void comparison(File aFile, String aCommand) throws InterruptedException {
